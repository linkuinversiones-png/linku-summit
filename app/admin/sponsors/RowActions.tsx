...
export default function RowActions({ id, active }: { id: string; active: boolean }) {
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => start(async () => toggleSponsorActive(id, !active))}
        disabled={pending}
      ...
      <button
        type="button"
        onClick={() => {
          if (!confirm('¿Borrar este sponsor? El logo en Storage se queda (puedes borrarlo desde el dashboard).')) return;
          start(async () => deleteSponsor(id));
        }}
        disabled={pending}
